# Пользователи
Подраздел ***Пользователи** предназначен для добавления, просмотра и редактирования данных о пользователях системы QR-Passport.
...
Для просмотра полной информации по пользователю необходимо выбрать соответствующую запись из общего списка.

При нажатии на имя пользователя осуществляется переход в карточку пользователя, где доступны следующие блоки информации:

* Учетные данные – имя пользователя и пароль (отображается в зашифрованном виде с использованием надежного алгоритма хеширования).
...
* Персональная информация – ФИО, должность, департамент, email и часовой пояс.

{% note info "" %}

Часовой пояс настраивается индивидуально для каждого сотрудника.

{% endnote %}

* Права доступа – настройка прав доступа, отметка о статусе активности учётной записи.

{% note info "" %}

Снятие отметки _Активный_ является альтернативой удалению учётной записи.

{% endnote %}

* Важные даты – позволяет отслеживать активность использования системы.
